...
#include "MemMap.hpp"
FUNC(Std_ReturnType, MEMIF_CODE) EcuabMemIf_Read(uint8 DeviceIndex, uint16 BlockNumber, uint16 BlockOffset, MemIf_DataPtr_pu8 DataBufferPtr, uint16 Length);
FUNC(Std_ReturnType, MEMIF_CODE) EcuabMemIf_Write(uint8 DeviceIndex, uint16 BlockNumber, MemIf_DataPtr_pu8 DataBufferPtr);
FUNC(Std_ReturnType, MEMIF_CODE) EcuabMemIf_InvalidateBlock(uint8  DeviceIndex, uint16 BlockNumber);
FUNC(Std_ReturnType, MEMIF_CODE) EcuabMemIf_EraseImmediateBlock(uint8 DeviceIndex, uint16 BlockNumber);
FUNC(void, MEMIF_CODE) EcuabMemIf_Cancel(uint8 DeviceIndex);
FUNC(MemIf_StatusType, MEMIF_CODE) EcuabMemIf_GetStatus(uint8 DeviceIndex);
FUNC(MemIf_JobResultType, MEMIF_CODE) EcuabMemIf_GetJobResult(uint8 DeviceIndex);

#if(MEMIF_VERSION_INFO_API == STD_ON)
void EcuabMemIf_GetVersionInfo(P2VAR(Std_VersionInfoType, AUTOMATIC, MEMIF_APPL_DATA) VersionInfoPtr);
#endif

FUNC(void, MEMIF_CODE) EcuabMemIf_SetMode(MemIf_ModeType Mode);
