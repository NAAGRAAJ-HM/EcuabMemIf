...
#error "Version numbers of EcuabIfMem.c and infEcuabIfMem.hpp are inconsistent!"
#endif

#if((MEMIF_TYPES_SW_MAJOR_VERSION != (3U)) || (MEMIF_TYPES_SW_MINOR_VERSION != (3U)))
#error "Version numbers of EcuabIfMem.c and Types_EcuabIfMem.hpp are inconsistent!"
#endif

#if((MEMIF_CFG_MAJOR_VERSION != (5U)) || (MEMIF_CFG_MINOR_VERSION != (2U)) )
#error "Version numbers of EcuabIfMem.c and CfgEcuabIfMem.h are inconsistent!"
#endif
...
MEMIF_LOCAL FUNC(Std_ReturnType, MEMIF_PRIVATE_CODE) MemIf_IsBitSet(
      uint8            Mask
   ,  MemIf_StatusType Status
){
   return(
      ((Mask & (1u << (uint8)Status)) > 0)
      ?  E_OK
      :  E_NOT_OK
   );
}
...
#if(MEMIF_DEV_ERROR_DETECT == STD_ON)
   if(E_NOT_OK == MemIf_DetChkDeviceIndex(DeviceIndex)){
      errorId = MEMIF_E_PARAM_DEVICE;
   }
   else
#endif
   {
      retVal = MemIf_MemHwaApis[DeviceIndex].Read(
            BlockNumber
         ,  BlockOffset
         ,  DataBufferPtr
         ,  Length
...
#endif
   {
      retVal = MemIf_MemHwaApis[DeviceIndex].Write(
            BlockNumber
         ,  DataBufferPtr
      );
   }
...
FUNC(MemIf_StatusType, MEMIF_CODE) MemIf_GetStatus(uint8 DeviceIndex){
   ...
   if(
      ...
      && (DeviceIndex >= MemIf_NumberOfDevices)
   ){
      errorId = MEMIF_E_PARAM_DEVICE;
   }
   else
   ...
   if(MEMIF_BROADCAST_ID == DeviceIndex){
      for(
         MemIf_LoopCounter = 0;
         MemIf_LoopCounter < MemIf_NumberOfDevices;
         MemIf_LoopCounter++
      ){
         MemIf_StatusMask |= (uint8) ((uint8)1u << (uint8) MemIf_MemHwaApis[MemIf_LoopCounter].GetStatus());
      }
           if(E_OK == MemIf_IsBitSet(MemIf_StatusMask, MEMIF_UNINIT        )){retVal = MEMIF_UNINIT;}
      else if(E_OK == MemIf_IsBitSet(MemIf_StatusMask, MEMIF_BUSY          )){retVal = MEMIF_BUSY;}
      else if(E_OK == MemIf_IsBitSet(MemIf_StatusMask, MEMIF_BUSY_INTERNAL )){retVal = MEMIF_BUSY_INTERNAL;}
      else                                                                   {retVal = MEMIF_IDLE;}
   }
   else{
